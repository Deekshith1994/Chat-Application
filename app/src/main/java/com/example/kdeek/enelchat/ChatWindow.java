...
public class ChatWindow extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getExtras()!=null){
            ...
            convWith.setText(LoginActivity.userArrayList.get(user).getFullName());
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot postSnapShot: dataSnapshot.getChildren()){
                    Log.d("test", postSnapShot.toString());
                        if (postSnapShot.getKey().equals(LoginActivity.curUser)) {
                                ...
                                if(msgofConv.getKey().equals(user)){
                                    for(DataSnapshot msgsfromThisUser : msgofConv.getChildren()){
                                        ...
                                        if(!msg.isRead) {
                                            Msg newMsg = msg;
                                            newMsg.setRead(true);
                                            myConvRef.child(LoginActivity.curUser).child(user).child(msg.ID).setValue(newMsg);
                                        }
                ...
                if(msgArrayList.size()!=0) {
                    ...
                    chatAdapter adapter = new chatAdapter(ChatWindow.this, msgArrayList, new chatAdapter.OnItemClickListener() {
                        ...
                        public void onItemClicked(int position) {
                            myConvRef.child(LoginActivity.curUser).child(user).child(msgArrayList.get(position).ID).removeValue();
        ...
        send.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String authorName = "";
                    ...
                    try{
                        authorName= LoginActivity.userArrayList.get(LoginActivity.curUser).getFullName();
                    }catch (Exception e){
                        authorName= FindContacts.usenames.get(LoginActivity.curUser).getFullName();
                    }

                Msg msg = new Msg(LoginActivity.curUser,authorName,msgText.getText().toString());
                String MsgID = UUID.randomUUID().toString();
                msg.ID = MsgID;
                msg.setRead(false);
                myConvRef.child(user).child(LoginActivity.curUser).child(MsgID).setValue(msg);
                myConvRef.child(LoginActivity.curUser).child(user).child(MsgID).setValue(msg);
